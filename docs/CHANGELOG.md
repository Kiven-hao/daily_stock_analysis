...
- [修复] API 错误响应不再泄露内部异常详情（`str(e)`），统一返回通用错误消息，防止攻击者通过错误信息探测系统内部结构
- [修复] 启用认证时 `/docs`、`/redoc`、`/openapi.json` 受认证保护，防止未授权用户访问 API 文档获取接口结构
- [修复] SQLite PRAGMA `busy_timeout` 设置改为显式整数校验，消除 f-string SQL 注入风险
